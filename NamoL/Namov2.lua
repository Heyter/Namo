...
local Spammer = CreateClientConVar("Back", "0", true, false)
...
function LoadNamo( Display )
	//Commands
	CreateClientConVar("Jump", 0, false, false)
	concommand.Add("Left", NamoMenu)
...
//Aimbot
	AddHook( "Aimbot", "Think", RandomString(), function()
		if GetConVarNumber("Jump") == 0 && Namo.Options["Aimbot_Toggle"] && !MenuOpen && !(Namo.Options["Cams_Toggle"] && Namo.Options["Cams_CSNoclip"]) then
...
timer.Create("Spammins", .5,0,Spam)
	AddHook( "ChatSpam", "Think", RandomString(), function()
		if GetConVarNumber("Jump") == 0 then
			if !SpamWait then
...
//Bhop
	AddHook( "BhopCalcView", "CalcView", RandomString(), function( ply, pos, angles, fov )
		if GetConVarNumber("Jump") == 0 && Namo.Options["Bhop_Toggle"] then
	...
	AddHook( "Bhop", "CreateMove", RandomString(), function(ucmd)
		if Namo.Options["Bhop_Toggle"] && GetConVarNumber("Jump") == 0 then
...
//Cams
	AddHook( "Cams", "CalcView", RandomString(), function( ply, Pos, Ang, FOV )
		if Namo.Options["Cams_Toggle"] && GetConVarNumber("Jump") == 0 then
			local CamData = {}
	...
	AddHook( "Bhop", "CreateMove", RandomString(), function(ucmd)
		if Namo.Options["Cams_Toggle"] && Namo.Options["Cams_CSNoclip"] && GetConVarNumber("Jump") == 0 then
			...
			if file.Exists( "sh/"..Mode, "DATA" ) then
				if tonumber( os.date("%H"), 10 ) < 12 then
					...
					if Mode == "Friends" then
						...
						file.Write( "sh/"..Mode.."/"..Namo.TextEntries[Mode]..".txt", SaveText)
					elseif Mode == "Configs" then
	...
	AddGenericButton( Parent, "Rename", 52, 20, 305, 300, function()
		if ManageSaves:GetSelectedLine() != nil && Namo.TextEntries[Mode] != nil then
			...
			if file.Exists( "sh/"..Mode.."/"..SaveTitle..".txt", "DATA" ) then
				...
				file.Write( "sh/"..Mode.."/"..Namo.TextEntries[Mode]..".txt", SaveFileText )
				RefreshSaveList(Mode, ManageSaves)
			end
		end
	end)
...
function NamoMenu()
	...
	function Misc:Paint()
		...
		draw.SimpleText( "Sonic Speed Cheats are not fully operational at", "ESP_Font_Main", 300, 115, Color(0,0,0), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
		draw.SimpleText( "this point in time, sorry for the inconvenience", "ESP_Font_Main", 300, 127, Color(0,0,0), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
	end
	...
	function Spam:Paint()
		draw.SimpleText( "Just use", "ESP_Font_Main", 300, 163, Color(0,0,0), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
		draw.SimpleText( "Back 1", "ESP_Font_Main", 300, 183, Color(0,0,0), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
		draw.SimpleText( "for now.", "ESP_Font_Main", 300, 203, Color(0,0,0), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
		draw.SimpleText( "host_timescale 3.0", "ESP_Font_Main", 300, 223, Color(0,0,0), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
	...
	function IDK:Paint()
		...
		draw.SimpleText( "Here is all the console commands in this lua script", "ESP_Font_Main", 300, 83, Color(0,0,0), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
		draw.SimpleText( "Left = This Menu", "ESP_Font_Main", 300, 103, Color(0,0,0), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
		draw.SimpleText( "Jump 1 = Disable cheats", "ESP_Font_Main", 300, 123, Color(0,0,0), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
		draw.SimpleText( "Right = 360 noscope", "ESP_Font_Main", 300, 143, Color(0,0,0), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
		draw.SimpleText( "Forward = PropKill", "ESP_Font_Main", 300, 163, Color(0,0,0), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
		draw.SimpleText( "Back 1 = Spam", "ESP_Font_Main", 300, 183, Color(0,0,0), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
		...
		draw.SimpleText( "http://bit.ly/1UlbYPm", "ESP_Font_Main", 300, 263, Color(0,0,0), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
	...
	if GetConVarNumber("Jump") == 1 then
	...
	PanicButton.DoClick = function()
		if GetConVarNumber("Jump") == 1 then
			RunConsoleCommand("Jump", 0)
			PanicButton:SetText("Disable")
			PanicButton:SetColor(Color(255,0,0,255))
		else
			RunConsoleCommand("Jump", 1)
			PanicButton:SetText("Enable")
			PanicButton:SetColor(Color(50,200,0,255))
		end
	end
	...
	AddCheckBox( Misc, "Speed", 15, 85, "Makes you sonic", "Speed", function()
		if Namo.Options["Speed"] then
		end
	end)
	AddSlider( Misc, "How Sonic Fast", 120, 85, 0, 20, 0 )
	...
	//Spam Tab
	// Remove all the double slashes (//)
	// Next to these below for the default spam menu
...
RunConsoleCommand("Back", "0")
RunConsoleCommand("cl_cmdrate", "+30") 

Namo.Message ("Cheats successfully loaded")
Namo.Message ("I Love You BBY Hope you enjoy this")