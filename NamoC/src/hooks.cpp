...
void __stdcall hooked_FrameStageNotify(int curStage)
{
	...
	if (dolel == 1)
	{
		...
		success = utils::file::ReadWaveFile("C://Namo//musicinput.wav", g_pMicInputFileData, g_nMicInputFileBytes, a, b, c);
...
class CNamoListener : public ILoggingListener
{
public:
...
std::string m_hooks::HookStuff()
{
	...
	LoggingSystem_RegisterLoggingListener(&g_NamoListener);
